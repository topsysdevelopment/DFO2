...
# SDDP short run for CFDP inititalization
sample_mode = 'unique'#'multi_random'#
nCuts = 5  # forward passes for SDDP

algo_optim_SDDP = SDDP(model, stateVariable, solver, sample_mode)
algo_optim_SDDP.optimize(nCuts)


# CFDP algorithm
nCuts = 2000#/size  # forward passes in simulation
sample_mode = 'unique'#
algo_optim = CFDP(model, stateVariable, solver, sample_mode)
algo_optim.initialize(algo_optim_SDDP)
algo_optim_SDDP.close()

algo_optim.optimize(nCuts, n_simulate = 100)
